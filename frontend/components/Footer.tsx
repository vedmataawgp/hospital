    ...
    <footer className="bg-[#0A2647]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            {/* CONTRAST FIX: text-slate-300 instead of text-white/50 */}
            ...
            <div className="flex items-center gap-3">
              {socials.map(s => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="w-8 h-8 rounded-lg bg-white/10 border border-white/15 flex items-center justify-center text-slate-300 hover:text-white hover:bg-white/20 hover:border-white/30 transition-all duration-200"
                >
                  {s.icon}
                </a>
              ))}
            ...
            <ul className="space-y-3">
              {quickLinks.map(l => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="text-slate-300 text-sm hover:text-white transition-colors flex items-center gap-2 group"
                  >
                    <span className="block w-3 h-px bg-slate-500 group-hover:w-5 group-hover:bg-[#4A90D9] transition-all duration-200" />
                    {l.label}
                  </Link>
                </li>
              ))}
              ...
              {services.map(s => (
                <li key={s}>
                  <span className="text-slate-300 text-sm flex items-center gap-2">
                    <span className="block w-3 h-px bg-slate-600" />
                    {s}
                  </span>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4">
              {[
                {
                  icon: <i className="bi bi-geo-alt-fill text-[#4A90D9]" />,
                  text: "123 Medical Drive, Health City",
                },
                {
                  icon: <i className="bi bi-telephone-fill text-[#4A90D9]" />,
                  text: "+1 (800) 555-2273",
                },
                {
                  icon: <i className="bi bi-envelope-fill text-[#4A90D9]" />,
                  text: "info@medicare.com",
                },
              ].map((item, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="mt-0.5 flex-shrink-0">{item.icon}</span>
                  {/* CONTRAST FIX: text-slate-300 */}
                  <span className="text-slate-300 text-sm">{item.text}</span>
                </li>
              ))}
            </ul>

            <div className="mt-6 flex items-center gap-2 bg-[#E63946]/20 border border-[#E63946]/40 rounded-xl px-4 py-3">
              <div className="w-2 h-2 rounded-full bg-[#E63946] animate-pulse-soft flex-shrink-0" />
              {/* CONTRAST FIX: text-red-300 for visibility */}
              <span className="text-red-300 text-xs font-semibold">Emergency: 24/7</span>
              <a href="tel:911" className="ml-auto text-red-300 text-xs font-bold hover:text-white transition-colors">Call 911</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          {/* CONTRAST FIX: text-slate-400 instead of text-white/30 */}
          <p className="text-slate-400 text-xs">
            &copy; {new Date().getFullYear()} MediCare. All rights reserved.
          </p>
          <div className="flex items-center gap-5">
            {["Privacy Policy", "Terms of Service", "Accessibility"].map(l => (
              <a key={l} href="#" className="text-slate-400 text-xs hover:text-white transition-colors">{l}</a>
            ))}
          </div>
        </div>
      </div>
